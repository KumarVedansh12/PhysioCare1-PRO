...
if not SECRET_KEY or SECRET_KEY.startswith("replace-"):
    raise ImproperlyConfigured("DJANGO_SECRET_KEY must be set to a strong, private value in .env")

DEBUG = os.getenv("DJANGO_DEBUG", "0") == "1"
ALLOWED_HOSTS = env_list("DJANGO_ALLOWED_HOSTS", "localhost,127.0.0.1" if DEBUG else "")
if not ALLOWED_HOSTS:
    raise ImproperlyConfigured("DJANGO_ALLOWED_HOSTS must list the production host names")
...
if not DATABASE_URL:
    raise ImproperlyConfigured("DATABASE_URL must contain the Supabase PostgreSQL connection string")
...
if not all((database.hostname, database.username, database.path.lstrip("/"))):
    raise ImproperlyConfigured("DATABASE_URL is incomplete; copy the full URI from Supabase Connect")
...
DATABASES = {
    "default": {
        ...
        "PORT": database_port,
        "CONN_MAX_AGE": 0 if transaction_pooler else int(os.getenv("POSTGRES_CONN_MAX_AGE", "60")),
...
STORAGES = {
    ...
    "staticfiles": {"BACKEND": "django.contrib.staticfiles.storage.StaticFilesStorage" if DEBUG else "django.contrib.staticfiles.storage.ManifestStaticFilesStorage"},
}
